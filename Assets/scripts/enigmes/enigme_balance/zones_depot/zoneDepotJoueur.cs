...
public class ZoneDepotJoueur : MonoBehaviour
{
    ...
    /// Utilise ReferenceEquals car l'op̩rateur == d'Unity retourne true
    /// pour les objets d̩truits, ce qui emp̻cherait Remove de les trouver.
    /// </summary>
    public void SupprimerObjetDetruit(objetPesable objet)
    {
        for (int i = _objetsDeposes.Count - 1; i >= 0; i--)
        {
            if (object.ReferenceEquals(_objetsDeposes[i], objet))
            {
                ...
                Debug.Log($"[ZoneDepotJoueur] Objet ramass̩ d̩tect̩. Total={CalculerPoidsTotal()}");
                NotifierBalance();
                return;
            }
        }
